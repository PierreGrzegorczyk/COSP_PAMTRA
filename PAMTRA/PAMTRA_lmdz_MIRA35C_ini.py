...
pam.df.addHydrometeor(("rain",1.,  1 , Rho_rain , -99., -99., -99., -99. , 3, 1, "mono",-99.0, -99.0, -99.0, -99.0,2*r_rain,-99.0,"mie-sphere","lmdz_rain",-99.))
...
pam.df.addHydrometeor(("snow",AR_snow, -1 , Rho_snow, -99., -99., np.pi/4., 2. ,  3 ,1,"mono",-99.0, -99.0, -99.0, -99.0,2*r_snow,-99.0,"ss-rayleigh-gans_%.3f_%.3f_%.3f_%.3f"%tuple(ssrg_coefs),"lmdz_snow",-99.))
...
pam.df.addHydrometeor(("ice", AR_ice, -1 , Rho_ice,  -99,-99 ,np.pi/4, 2.  , 3 ,1, "mono", -99., -99., -99., -99., 2*r_ice, -99., "ss-rayleigh-gans", "heymsfield10_particles",-99.))
...
#_____________MIRA 35 C characteristics__________
v_max=13.29 #vit nyq
nfft=512#1024#512
Z_sensi=-60.3
Beam_width=0.52
...
pam.nmlSet['radar_pnoise0'] = Z_sensi
pam.nmlSet["radar_noise_distance_factor"] = 1 #r^k noise increse
...
if Run_spectra==True:
    ...
    dv0 = vel0[1]-vel0[0]
    pam.nmlSet['radar_pnoise0'] = Z_sensi+10*np.log10(dv0*nfft)
...
if Run_pamtra==True:
    ...
    if Run_parall==True:
        ...
        pam.runPamtra(35.1,checkData=False)
