...
alpha_list = np.array([0.001,0.001,0.003, 0.004, 0.005, 0.007, 0.0185, 0.05])
alpha_list = np.array([0.0005,0.0005,0.001,0.003, 0.004, 0.005, 0.007, 0.0185, 0.05])
...
for alpha in alpha_list:
    ...
    for Radar_type in Radar_type_list:
        ...
        pam.df.addHydrometeor(('snow', AR_snow, -1,  -99., alpha, 2,  -99.,  -99., 13, 200, 'exp', -99.0, -99.0, -99.0, -99.0, 20e-5,  0.5,"ss-rayleigh-gans_%.3f_%.3f_%.3f_%.3f"%tuple(ssrg_coefs),'lmdz_snow',0.0))
        #pam.df.addHydrometeor(('snow', AR_snow, -1,  -99., alpha, 2,  -99.,  -99., 3, 200, 'exp_cosmo_snow', -99.0, -99.0, -99.0, -99.0, 20e-5,  0.05,"ss-rayleigh-gans_%.3f_%.3f_%.3f_%.3f"%tuple(ssrg_coefs),'lmdz_snow',0.0))
    ...
    #__________namelist___________
        ...
        print("Ze",",",alpha,",",pam.r["Ze"][0][0][0][0][0][0])
        dBZ[j,i]=pam.r["Ze"]
        j+=1
    i+=1
...
lambd=[44.721359549995789  , 63.245553203367585   ,  109.54451150103321   ,   126.49110640673517  ,     141.42135623730951     ,   167.33200530681509     , 272.02941017470886       ,447.21359549995793 ]

plt.figure('Ze',figsize=(14,5))
plt.plot(lambd,dBZ[2][1:],label="MRR")
plt.plot(lambd,dBZ[1][1:],label="Mira")
plt.plot(lambd,dBZ[0][1:],label="Basta")
plt.xlabel('$\lambda$ [m$^{-1}$]')
plt.ylabel("Ze [dBZ]")
plt.legend()
plt.tight_layout()
plt.savefig("/home/grzegorc/AWACA/Tests_DFR/Ze_exp.png")




plt.figure('DFR',figsize=(10,5))
plt.plot(lambd,dBZ[1][1:]-dBZ[0][1:],label='Z$_{Ka}$-Z$_{W}$ (Mira-Basta)')
plt.plot(lambd,dBZ[2][1:]-dBZ[1][1:],label='Z$_{K}$-Z$_{Ka}$ (MRR-Mira)')
plt.legend()
plt.xlabel('$\lambda$ [m$^{-1}$]')
plt.ylabel("Ze [dB]")
plt.tight_layout()
plt.savefig("/home/grzegorc/AWACA/Tests_DFR/DFR_exp.png")
plt.show()
