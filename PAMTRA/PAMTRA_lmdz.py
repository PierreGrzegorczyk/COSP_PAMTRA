...
if "Ground"=="Aircraft":
    ...
    if 'up'=='down' or 'up'=='both':
        pam.nmlSet['radar_attenuation']='top-down'
        pam.p['obs_height'][:,:,0] = np.repeat(nc_aircraft['radar_altitude'][:][::step][isel:jsel, np.newaxis], ncol, axis=1)#0.0

    elif 'up'=='up':
        pam.nmlSet['radar_attenuation']='bottom-up'
        pam.p['obs_height'][:,:,1] = np.repeat(nc_aircraft['radar_altitude'][:][::step][isel:jsel, np.newaxis], ncol, axis=1)#0.0
...
output_file = "../output/Earthcare_traj2.nc"#../Ouput_golden_case_D17_v6_ssrga_spectra_Ka.nc"
