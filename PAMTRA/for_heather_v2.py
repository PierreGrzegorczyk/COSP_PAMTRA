...
#print(spl_beta_agg)
#data = np.load('/home/grzegorc/AWACA/ssrga_coeffs/spl_beta_agg.npy')

riming_deg=["0.0","0.1","0.2","0.5","1.0","2.0"]
l=5
#print(df.head())  # first 5 rows
df = pd.read_csv(
    "/home/grzegorc/AWACA/ssrga_coeffs/ssrga_coeffs_simultaneous_" + riming_deg[0] + ".csv",
    skiprows=4
)
...
r_snow_list=df['Diam_max']
beta_list=df['beta']
gamma_list=df['gamma']
kappa_list=df['kappa']
zeta_list=df['zeta']
alpha_eff_list=df['alpha_eff']
...
for r_snow in r_snow_list:
    ...
    for Radar_type in Radar_type_list: 
        ...
        AR_snow=alpha_eff_list[i]
        #kappa_beta_gamma_zeta
        ssrg_coefs = [kappa_list[i],beta_list[i],gamma_list[i],zeta_list[i]]
